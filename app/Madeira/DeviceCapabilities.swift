...
/// Runs the Metal 4 self test inside the app, on the device, and puts its stage
/// table in LogStore.
///
/// The macOS run of the same harness reports SKIP, which says nothing about an
/// iPad M-series GPU. This is the device's answer, and it comes from the same
/// `mr_mtl4_milestone_run` the layer is built from -- there is no app-side copy
/// of the checks, only this call.
///
/// Opt-in on purpose. Running it at every launch would put a Metal 4 device and
/// command buffer in front of the normal runtime for no reason, and a self test
/// that fires by itself is a self test that eventually breaks a game launch.
/// Two ways to trigger it, both explicit:
///
///   - launch argument:  --metal4-selftest
///   - a default:        defaults write <bundle-id> MadeiraMetal4SelfTest -bool YES
///
/// Neither is set by default, so a normal run never reaches this code.
enum Metal4Diagnostics {
    private static let defaultKey = "MadeiraMetal4SelfTest"
    private static let launchArgument = "--metal4-selftest"

    /// True when the environment asks for the self test.
    static var isRequested: Bool {
        if ProcessInfo.processInfo.arguments.contains(launchArgument) {
            return true
        }
        return UserDefaults.standard.bool(forKey: defaultKey)
    }

    /// Runs the self test if it was requested, and does nothing otherwise.
    static func runIfRequested() {
        guard isRequested else { return }
        runSelfTest()
    }

    /// Runs it unconditionally. Safe to call from a debug action or a settings
    /// toggle later; the rows come from the harness verbatim.
    static func runSelfTest() {
        LogStore.shared.log("[metal4] self test: starting")
        guard let rows = madeira_metal4_selftest() else {
            // A nil return means the harness could not run at all, which is a
            // result and must not be logged as an empty table.
            LogStore.shared.log("[metal4] self test: no table returned", level: .error)
            return
        }
        let table = String(cString: rows)
        for line in table.split(separator: "\n", omittingEmptySubsequences: true) {
            LogStore.shared.log("[metal4] \(line)")
        }
        LogStore.shared.log("[metal4] self test: done")
    }
}
